...
const defaultColumns = fromJS([
  ...
  { id: 'GENERAL', uuid: uuid(), params: {} },
  { id: 'NOTIFICATIONS', uuid: uuid(), params: {} },
  { id: 'DIRECT', uuid: uuid(), params: {} }
]);

const hydrate = (state, settings) => state.mergeDeep(settings).update('columns', (val = defaultColumns) => {
  let columns = val || defaultColumns;

  if (!columns.some(column => column.get('id') === 'GENERAL')) {
    const homeIndex = columns.findIndex(column => column.get('id') === 'HOME');
    const generalColumn = fromJS({ id: 'GENERAL', uuid: uuid(), params: {} });
    columns = homeIndex >= 0 ? columns.insert(homeIndex + 1, generalColumn) : columns.push(generalColumn);
  }
